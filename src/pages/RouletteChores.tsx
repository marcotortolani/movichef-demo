...
export default function RouletteChores({
  ...
  return (
    ...
    >
                ...
                .map((_, i) => (
                  <div
                    ...
                    className={`${
                      ...
                      choresSelected?.includes(i + 1) ? 'filter grayscale' : ''
      ...
      <div
        className={`${
          showChoreSelected ? ' flex scale-100 ' : ' hidden scale-0 '
        } z-50 bg-black/20 backdrop-blur-sm w-full h-full absolute transition-all duration-200 ease-in-out flex items-center justify-center`}
      >
        <div className=" w-5/6 aspect-square flex flex-col items-center justify-center gap-4 bg-primaryLighter shadow-lg shadow-black/30 rounded-2xl">
          {catWheel > 0 && (
            <img
              className=" w-1/2"
